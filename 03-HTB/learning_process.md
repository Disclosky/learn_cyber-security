## LEARNING PROCESS

* Forum: https://forum.hackthebox.com
* Discord: https://discord.gg/hackthebox

---
### Sunday, 19 July 2026

We will become a good penetration tester only through considerable practice.

we have to practice active learning

we have to learn how to find, choose, and adapt the information we need.

`20 * ________+ ________ = 65535`

```
65535 : 20     = 3276.75
[3276] * 20    = 65520   or   0.75 * 20 = 15
65535 - 65520  = [15]
------------------------
20 * 3276 + 15 = 65535
```

"Think outside the box"

Occam's Razor is one of the central principles of modern scientific theory. The principle is based on the following definition:
>The most straightforward theory is preferable to all others of several sufficient possible explanations for the same state of facts. In other words: The simplest explanation is always the most probable.<br>
_Di antara berbagai kemungkinan penjelasan yang sama-sama memadai untuk suatu fakta yang sama, teori yang paling lugas lebih diutamakan dibandingkan yang lain. Dengan kata lain: Penjelasan yang paling sederhana selalu merupakan penjelasan yang paling mungkin benar._

- The simplest explanation for the best approach to penetration testing is that we work with the information we can get.

talent itself is not innate, it's pure hard work

constant or persistent

If we feel good and we know we will reach our goal, we will be successful.

`know your goal`

> Optional Exercise:
Write down the goal you want to achieve with this course as precisely as possible. Try to break it down and describe it in 500 words at most.

#### Learning Efficiency

..
